...
(function () {
    ...
    function setCustomTooltipPosition (tooltipModel, tooltipElements, eventPosition) {
      var tooltipEl = tooltipElements.tooltipEl
    ...
    function legendClickCallback (chart, label, dataset) {
      ...
      // save DMA settings
      var rememberMetric = ['DMA50', 'DMA200', 'Index DMA200', 'Index DMA50', 'GPM', 'NPM', 'OPM']
    ...
    function drawLegend (chart) {
      ...
      chart.data.datasets.map(function (dataset) {
        ...
        else if (dataset.yAxisID === 'y-axis-right') rightLabels.push(dataset.label)
    ...
    function drawChart (target, state) {
      ...
      Chart.Tooltip.positioners.setCustomTooltipPosition = function (elements, eventPosition) {
        var tooltipModel = this
        // default elements is elements used by Chart.js
        return setCustomTooltipPosition(tooltipModel, tooltipElements, eventPosition)
      }
    ...
    function updateChart (chart, state) {
      ...
      if (!chartData.scales['y-axis-left']) {
        if (chart.options.scales['y-axis-left']) delete chart.options.scales['y-axis-left']
      } else {
    ...
    function updateMetricsIndicator (metrics) {
      ...
      var focusButtons = document.querySelectorAll('#company-chart-metrics > button')
      var isInFocus = false
      ...
      if (!isInFocus) {
        ...
        var moreActiveButton = document.getElementById('company-chart-metrics-more-active')
        moreActiveButton.value = metrics
        moreActiveButton.innerText = activeMetricName
        moreActiveButton.classList.remove('hidden')

        // hide in dropdown
        var dropdown = document.querySelector('#company-chart-metrics .dropdown-menu')
    ...
    async function handleActiveMetrics (chart, state, optionMetrics) {
      ...
      var midPeriodCharts = ['Price to Earning', 'Price to book value', 'EV Multiple', 'Market Cap to Sales']
      var isMaxPeriod = metrics.filter(function (metric) {
        return maxPeriodCharts.includes(metric)
      }).length >= 1
      var isMidPeriod = metrics.filter(function (metric) {
    ...
    async function setUpEverything () {
      ...
      let firstButton = document.querySelector('#chart button[name="metrics"]')
      var state = {
        days: 365,
